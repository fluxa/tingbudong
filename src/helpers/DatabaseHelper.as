package helpers
{
	...
	public class DatabaseHelper
	{
		...
		private function searchDefinition(string:String):Array
		{
			...
			sql = "SELECT * FROM words WHERE ({column} LIKE '{string} %' OR {column} LIKE '% {string}' OR {column} LIKE '% {string} %' OR {column} LIKE '%|{string} %' OR {column} LIKE '% {string}|%')";
			sql = sql.replace(/{column}/g, col);
			sql = sql.replace(/{string}/g, string);
		...
		{
			var sql:String = "INSERT INTO grammar VALUES(null, :groupid, :description, :notes, :tags, :decayindex, :knowledge, :lasttimes, :lastcorrect, :alltimes);";
			
			var dbStatement:SQLStatement = new SQLStatement();
			dbStatement.text = sql;
			dbStatement.parameters[":groupid"] 				= gram.groupid;
			dbStatement.parameters[":description"] 			= gram.description;
			dbStatement.parameters[":notes"] 				= "";//gram.notes;
		...
		public static function updateGrammar(gram:Grammar=null):Boolean{
			if(gram)
			{
				var sql:String = "UPDATE grammar SET description = :description, notes = :notes, groupid = :groupid, lasttimes = :lasttimes, lastcorrect = :lastcorrect, alltimes = :alltimes, decayindex = :decayindex, knowledge = :knowledge, tags = :tags WHERE id = " + gram.id.toString();
				var dbStatement:SQLStatement = new SQLStatement();
				dbStatement.text = sql;
				dbStatement.parameters[":description"] 			= gram.description;
				dbStatement.parameters[":notes"] 				= gram.notes;
		...
		public static function checkDuplicateGrammar(description:String):Boolean
		{
			
			var all:Array = DatabaseHelper.getInstance().getAllGrammar();	
			for each(var g:Grammar in all) { if(g.description == description) return true; }
			return false;
		}
		...
		private function getGrammar(sql:String):Array
		{
			...
			for each(var rawGram:Object in data){
				...
				gram.id 			= rawGram.id;
				gram.description 	= rawGram.description;
				gram.notes 			= rawGram.notes;
				gram.groupid		= rawGram.groupid;
				gram.sentences 		= DatabaseHelper.getInstance().getSentencesFromGrammar(rawGram.id);
		...
		public function getWordsWithTags(tags:Array):Array
		{
			...
			var allWords:Array = getAllWords();
			...
			return allWords.filter(
				...
				{
					var word:Vocab = element;
					for each(var tag:String in tags)
					{
						
						if(word.tagArray.indexOf(tag) >= 0){
							if(tag == "grammar"){
							trace("stop");
		...
		public function getLevelofTag(tag:String):Number{
		
			var allWords:Array =  getWordsWithTags(new Array(tag));
			return getLevelOfWords(allWords);
		}
		public function getLevelOfWords(arr:Array):Number{

			var level1:Number = XMLParsingHelper.getAllForWords(XMLParsingHelper.IKNOWTHESE, arr, 	Vocab.CURRENT_KNOWLEDGE);
			var level2:Number = XMLParsingHelper.getAllForWords(XMLParsingHelper.ALMOST, arr, 		Vocab.CURRENT_KNOWLEDGE);
			var level3:Number = XMLParsingHelper.getAllForWords(XMLParsingHelper.ALITTLE, arr, 		Vocab.CURRENT_KNOWLEDGE);
			var level4:Number = XMLParsingHelper.getAllForWords(XMLParsingHelper.WHATSTHAT, arr, 	Vocab.CURRENT_KNOWLEDGE);
			var level5:Number = XMLParsingHelper.getAllForWords(XMLParsingHelper.NOIDEA, arr, 		Vocab.CURRENT_KNOWLEDGE);
			
			var level:int 		= 	XMLParsingHelper.NOIDEA;
			var res:Number 	=	0;
			if(level2 >= level1){
				res 	= level2;
				level 	= XMLParsingHelper.ALMOST;
			}else{
				res 	= level1;
				level 	= XMLParsingHelper.IKNOWTHESE;
			}
			if(level3 >= res){
				res 	= level3;
				level 	= XMLParsingHelper.ALITTLE;
			}
			if(level4 >= res){
				res 	= level4;
				level 	= XMLParsingHelper.WHATSTHAT;
			}
			if(level5 >= res){
				res 	= level5;
				level 	= XMLParsingHelper.NOIDEA;
			}
			return level;
		...
		public function getSentencesFromGrammar(id:String=null):Array
		{
		...
		public function getGrammarByDescription(txt:String):Array{
			var sql:String = "SELECT * FROM grammar WHERE description ='" + txt+"'";
			return DatabaseHelper.getInstance().getGrammar(sql);
