package mediator
{
	...
	public class QuizManager extends BaseMediator
	{
		...
		{
			var sessionsArr:Array 	= DatabaseHelper.getInstance().getSessions();
			var tagsArr:Array 		= DatabaseHelper.getInstance().getTags();
			
			//fill datagrids
			view.sessionselctor.dataProvider 	= getSessionVisuals(sessionsArr);
		...
		public function setSettings():void{
			...
			view.updateSessions_txt.text 	= DatabaseHelper.getInstance().getSessions().length.toString()+" sessions and "+DatabaseHelper.getInstance().getAllWords().length.toString()+" words";
		...
		private function getTagVisuals(arr:Array):Array{
			...
			for each(var str:String in arr){
				obj = new Object();
				obj.name 	= str;
				obj.wordlen = DatabaseHelper.getInstance().getWordsWithTags(new Array(str)).length;
				obj.level 	= DatabaseHelper.getInstance().getLevelofTag(str);
				newArr.push(obj);
			}
			return newArr;
		}
